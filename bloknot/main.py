...
tutorial = tkinter.messagebox.askquestion('Configuration', 'Would you like to read a quick tutorial?').lower() == "yes"
...
en = tkinter.messagebox.askquestion('Configuration', 'Would you like to continue in English?').lower() == "yes"
...
text_w = tk.Text(root, wrap = "word", bg="black",font=font, fg="white")
...
with open("text.json",encoding="utf8") as f:
    fr = json.load(f)
text = fr["english" if en else "ukrainian"]
indx = -1
def typer():
    global indx
    indx += 1
    if indx < len(text):
        ...
        tkinter.messagebox.showinfo("Are you ready?", "Are you ready to show who's the real Labyrinth Legend?")
